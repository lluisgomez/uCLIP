...
def get_scores(all_txt_embs, all_img_embs):
    all_txt_embs = torch.stack(all_txt_embs)  # Shape: (N, 5000, 768)
    all_img_embs = torch.stack(all_img_embs)  # Shape: (N, 1000, 768)
    # Normalize in batch
    all_txt_embs = F.normalize(all_txt_embs, dim=-1)  # Shape: (N, 5000, 768)
    all_img_embs = F.normalize(all_img_embs, dim=-1)  # Shape: (N, 1000, 768)
    # Transpose all_img_embs to (N, 768, 1000)
    all_img_embs_t = all_img_embs.transpose(1, 2)
    # Use batched matrix multiplication
    all_scores_tensor = torch.bmm(all_txt_embs, all_img_embs_t)  # Shape: (N, 5000, 1000)
